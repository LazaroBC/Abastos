...
SELECT nombre AS Empleado, oficina, contrato AS [Fecha de contrato]  FROM empleados;
...
SELECT idfab, idproducto, descripcion, productos.precio  FROM productos;
...
SELECT idfab + idproducto AS Codigo, descripcion, productos.precio  FROM productos;
...
SELECT idfab,idproducto,descripcion, existencias, precio, (existencias*precio) AS valoracion FROM productos;
...
SELECT idfab,idproducto,descripcion, existencias, precio, (precio* 1.18) AS [Precio Con IVA] FROM productos;
...
SELECT idfab,idproducto,descripcion, existencias, precio, ROUND((precio* 1.18),2) AS [Precio Con IVA] FROM productos;
...
SELECT idfab,idproducto,descripcion, existencias, precio, ROUND(precio* 1.18,2) AS [Precio Con IVA], STR(precio*1.18,6,2) AS [Con STR] FROM productos;
...
ROUND((precio* 1.18),2) AS [Precio Con IVA],  
STR(precio*1.18,6,2) AS [Con STR],  
existencias * ROUND((precio* 1.18),2) AS Valoracion,  
existencias * STR(precio*1.18,6,2)  -- esto da error porque no se puede  
                                    -- multiplicar un número por un caracter 
FROM productos;
...
SELECT nombre, MONTH(contrato) AS [Mes de contrato], YEAR(contrato) AS [Año de contrato] FROM empleados; 
...
SELECT numemp, nombre, contrato,                               GETDATE() AS Hoy            FROM  empleados;
...
SELECT numemp AS Código, nombre, CONVERT(varchar,contrato, 103) AS 'Fecha Contrato', CONVERT(varchar,GETDATE(),3) AS 'Fecha de hoy' FROM empleados; 
SELECT numemp, nombre, contrato, CONVERT(CHAR(10),contrato,103) AS Contrato,         CONVERT(CHAR(8),GETDATE(),3) AS Hoy            FROM empleados;
/* CHAR(10) es longitud fija y es mas rapido que VARCHAR (longitud variable, 50% más lento)*/

/*16.- Añade a la consulta anterior el número de días trabajados.Puedes practicar con las funciones de fechas.*/
SELECT numemp AS Código, nombre, CONVERT(varchar,contrato, 103) AS 'Fecha Contrato', CONVERT(varchar,GETDATE(),3) AS 'Fecha de hoy', DATEDIFF(DAY,contrato, GETDATE())    AS 'Días Trabajados' FROM empleados;
SELECT numemp, nombre, contrato, CONVERT(CHAR(10),contrato,103) AS Contrato,         CONVERT(CHAR(8),GETDATE(),3) AS Hoy,            DATEDIFF(day , contrato , GETDATE()) AS [Dias Trabajados] FROM empleados;
